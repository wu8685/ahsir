...
// resolveSpeaker turns the --as flag into the speaker identity sent with the
// message. Empty falls back to the OS username (then $USER) so shared-context
// attribution works with zero configuration; an explicit value wins so one
// machine can host several logical identities ("alice" vs "ci-bot").
// Self-claimed by design at the local-machine trust level — replaced by the
// verified principal when ingress auth lands.
func resolveSpeaker(asFlag string) string {
	if asFlag != "" {
		return asFlag
	}
...
// chatCmd: `ahsir chat <agent> "<message>" [--scheduler URL] [--context ID] [--as NAME] [--stream]`
func chatCmd(args []string) {
	...
	client := newSchedulerClient(*schedulerURL)
	speaker := resolveSpeaker(*speakerFlag)
	...
	// Both remaining modes submit via async + taskId. Default ("sync") wraps
	...
	if *asyncMode {
		// taskId alone on stdout — composable; the polling hint goes to
		// stderr like other informational output.
		fmt.Println(taskID)
		fmt.Fprintf(os.Stderr, "(queued; poll with `ahsir status %s %s`, transcript via `ahsir history %s %s`)\n", agent, taskID, agent, taskCtx)
		return
	}

	reply, err := client.WaitForTask(agent, taskID, taskCtx)
	if err != nil {
		fmt.Fprintf(os.Stderr, "chat failed: %v\n", err)
		os.Exit(1)
	}
