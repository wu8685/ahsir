...
func TestSessionPool_CapEvictsLRUOnPolicy(t *testing.T) {
	...
	// ctx-a's session must be closed. LookupOrCreate returns the gate
	// wrapper; the fake lives one layer in.
	if !s1.(*gatedSession).inner.(*fakePoolSession).isClosed() {
...
// gateProbeSession is a controllable Session: each Stream call registers its
// label (parsed from userText) in arrival order, then blocks until the test
// feeds one token into release. Lets tests hold a turn "in flight" and
// observe execution order.
type gateProbeSession struct {
	mu        sync.Mutex
	order     []string
	active    int
	maxActive int
...
func newGateProbeSession() *gateProbeSession {
	return &gateProbeSession{release: make(chan struct{})}
}

func (s *gateProbeSession) Stream(ctx context.Context, userText string) (<-chan Event, error) {
	s.mu.Lock()
	s.order = append(s.order, userText)
	s.active++
	...
	go func() {
		...
		s.active--
		s.mu.Unlock()
		ch <- EventText{Text: "done:" + userText}
		ch <- EventTurnDone{}
	}()
	return ch, nil
}

func (s *gateProbeSession) Turn(ctx context.Context, userText string) (string, error) {
...
func (s *gateProbeSession) SessionID() string { return "gate-probe" }
func (s *gateProbeSession) IsHealthy() bool   { return true }
func (s *gateProbeSession) Close() error      { return nil }

func (s *gateProbeSession) maxConcurrent() int {
	s.mu.Lock()
	defer s.mu.Unlock()
	return s.maxActive
}

func (s *gateProbeSession) executed() []string {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func newGatedTestPool(t *testing.T, probe *gateProbeSession) *SessionPool {
	t.Helper()
...
// drainStream runs one full turn through the pool-returned session,
// reporting the terminal error of Stream acquisition.
func drainStream(ctx context.Context, pool *SessionPool, contextID, label string) error {
	s, err := pool.LookupOrCreate(ctx, contextID)
	if err != nil {
		return err
	}
	ch, err := s.Stream(ctx, label)
	if err != nil {
		return err
	}
	for range ch {
...
// TestPoolQueueSerializesTurnsInOrder: concurrent turns on one context all
// run, in arrival order, with no busy rejection at default depth.
func TestPoolQueueSerializesTurnsInOrder(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		...
		go func(i int) {
			defer wg.Done()
			errs[i] = drainStream(ctx, pool, "ctx-fifo", fmt.Sprintf("t%d", i))
		}(i)
		time.Sleep(50 * time.Millisecond) // stagger arrivals deterministically
	}
	for i := 0; i < 3; i++ {
	...
	// The decisive assertion: turns must run one at a time. Without the
	// gate all three Streams overlap and this reads 3.
	if probe.maxConcurrent() != 1 {
		t.Fatalf("max concurrent turns = %d, want 1 (serialized)", probe.maxConcurrent())
	}
}

// TestPoolQueueOverflowFailsFast: with depth 1, the third concurrent turn
// (one running + one queued) fails immediately with the busy sentinel.
func TestPoolQueueOverflowFailsFast(t *testing.T) {
	probe := newGateProbeSession()
	pool := newGatedTestPool(t, probe)
	pool.SetQueueDepth(1)

	ctx := context.Background()
	var wg sync.WaitGroup
	// Turn 1: occupies the gate.
	wg.Add(1)
	go func() { defer wg.Done(); _ = drainStream(ctx, pool, "ctx-overflow", "t1") }()
	time.Sleep(50 * time.Millisecond)
	// Turn 2: waits in the queue.
	wg.Add(1)
	go func() { defer wg.Done(); _ = drainStream(ctx, pool, "ctx-overflow", "t2") }()
	time.Sleep(50 * time.Millisecond)

	// Turn 3: queue full → fail-fast busy, well before any turn completes.
	start := time.Now()
	err := drainStream(ctx, pool, "ctx-overflow", "t3")
	if err == nil {
		t.Fatal("expected busy error on full queue")
	}
	if !errors.Is(err, ErrTurnInFlight) {
		t.Fatalf("overflow error = %v, want ErrTurnInFlight", err)
	}
	if !strings.Contains(err.Error(), "agent busy:") {
		t.Fatalf("overflow error must carry the wire marker, got %v", err)
	}
	if time.Since(start) > 2*time.Second {
		t.Fatalf("busy rejection took %v — must fail fast, not queue", time.Since(start))
	}

	probe.release <- struct{}{}
	probe.release <- struct{}{}
	wg.Wait()
	if got := probe.executed(); len(got) != 2 {
		t.Fatalf("executed turns = %v, want exactly t1,t2", got)
	}
}

// TestPoolQueueCancelledWaiterNeverRuns: a queued waiter whose context is
// cancelled leaves the queue without executing.
func TestPoolQueueCancelledWaiterNeverRuns(t *testing.T) {
	probe := newGateProbeSession()
	pool := newGatedTestPool(t, probe)
	...
	var wg sync.WaitGroup
	wg.Add(1)
	go func() { defer wg.Done(); _ = drainStream(bg, pool, "ctx-cancel", "t1") }()
	time.Sleep(50 * time.Millisecond)

	cancelCtx, cancel := context.WithCancel(bg)
	waitErr := make(chan error, 1)
	go func() { waitErr <- drainStream(cancelCtx, pool, "ctx-cancel", "t2") }()
	time.Sleep(50 * time.Millisecond)
	cancel()
	...
	for _, label := range probe.executed() {
		if label == "t2" {
			t.Fatal("cancelled waiter must never execute")
		}
	}
}

// TestPoolQueueDepthZeroRestoresFailFast pins the escape hatch: queue_depth=0
// reproduces the pre-queue busy contract exactly.
func TestPoolQueueDepthZeroRestoresFailFast(t *testing.T) {
	probe := newGateProbeSession()
	pool := newGatedTestPool(t, probe)
	pool.SetQueueDepth(0)

	ctx := context.Background()
	var wg sync.WaitGroup
	wg.Add(1)
	go func() { defer wg.Done(); _ = drainStream(ctx, pool, "ctx-zero", "t1") }()
	time.Sleep(50 * time.Millisecond)

	if err := drainStream(ctx, pool, "ctx-zero", "t2"); !errors.Is(err, ErrTurnInFlight) {
		t.Fatalf("depth=0 collision error = %v, want ErrTurnInFlight", err)
	}
...
// TestPoolQueueDistinctContextsStayParallel: the gate is per-context — a held
// turn on one context must not delay another context at all.
func TestPoolQueueDistinctContextsStayParallel(t *testing.T) {
	probeA := newGateProbeSession()
	probeB := newGateProbeSession()
	pool := NewSessionPool(func(ctx context.Context, contextID, resumeID string) (Session, error) {
		if contextID == "ctx-a" {
			return probeA, nil
		}
	...
	var wg sync.WaitGroup
	wg.Add(1)
	go func() { defer wg.Done(); _ = drainStream(ctx, pool, "ctx-a", "a1") }()
	time.Sleep(50 * time.Millisecond)

	done := make(chan error, 1)
	go func() { done <- drainStream(ctx, pool, "ctx-b", "b1") }()
	go func() { probeB.release <- struct{}{} }()
	select {
	case err := <-done:
		if err != nil {
			t.Fatalf("ctx-b turn failed while ctx-a held: %v", err)
		}
	case <-time.After(3 * time.Second):
		t.Fatal("ctx-b turn blocked behind ctx-a's gate — gate must be per-context")
	}
