...
//
// Liveness is never written back here: an agent's effective status is
// derived from lastBeat on read (see GetStatus), so readers under RLock
// stay read-only and race-free.
...
func (h *HTTPHandler) handleAgents(w http.ResponseWriter, r *http.Request) {
	...
	case http.MethodPost:
		...
		// Agent cards are small; 1 MiB bounds a misbehaving (or malicious)
		// registration without affecting any legitimate card.
		if err := json.NewDecoder(http.MaxBytesReader(w, r.Body, 1<<20)).Decode(&card); err != nil {
