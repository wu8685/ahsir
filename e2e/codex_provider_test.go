...
func TestCodexProvider_E2E(t *testing.T) {
	...
	reply3, err := fix.sendMessageToAgent(
		"teacher",
		"msg-codex-teacher-3",
		"codex-provider-direct",
		"What is this teacher's favorite fruit? Reply with only the answer.",
	)
	if err != nil {
		t.Fatalf("teacher direct scheduler call: %v\n--- scheduler log ---\n%s", err, fix.schedulerLog())
	}
	if !strings.Contains(reply3, "papaya-5") {
		t.Fatalf("teacher direct reply should contain the teacher-only answer, got %q\n--- scheduler log ---\n%s", reply3, fix.schedulerLog())
	}
