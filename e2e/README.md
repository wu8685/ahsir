...
├── kill_recovery_test.go     ← SIGKILL'd claude → next turn resumes transparently
├── scheduler_restart_test.go ← full scheduler bounce → conversation memory survives
├── turn_timeout_test.go      ← runtime.timeout=1s → clear, timely error (no hang)
├── start_failure_test.go     ← pinned-port conflict fails fast; auto-port skips occupied
├── agent_lifecycle_test.go   ← agent new → list → chat → agent delete (the skill's main path)
├── observability_test.go     ← ahsir doctor / ahsir trace against a live stack
├── attempt_budget_test.go    ← delegation to a nonexistent agent terminates within budget
└── same_context_busy_test.go ← concurrent same-contextId → one success + one fast "agent busy" rejection
```
