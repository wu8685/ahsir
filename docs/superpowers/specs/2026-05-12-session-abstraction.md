...
4. **EVICTED entry GC** — **Resolved ✅**. `SessionPool.reapOnce` keeps inactive mappings bounded by `pool.evicted_ttl` (default `30d`) and `pool.max_evicted` (default `1000`, oldest inactive records deleted first). Persistence layer (`<workspace>/.a2a/sessions.json`) also respects this — the file is rewritten on GC.
