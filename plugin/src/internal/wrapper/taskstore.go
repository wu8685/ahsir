...
// StateOf reads a task's state under the store lock. Writers replace whole
// Task values via Save (never mutate stored objects in place), so this is the
// race-free way for the async execution path to check for cancellation.
func (ts *TaskStore) StateOf(id a2a.TaskID) (a2a.TaskState, bool) {
	ts.mu.RLock()
	defer ts.mu.RUnlock()
	task, ok := ts.tasks[id]
	if !ok {
