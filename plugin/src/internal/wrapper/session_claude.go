...
func buildProductionTransport(ctx context.Context, cfg SessionConfig, resumeID string) (*claudeTransport, error) {
	args := injectStreamJSONFlags(stripClaudePermissionBypassArgs(append([]string(nil), cfg.Args...)))
	if resumeID != "" {
	...
	killFn := func() error {
		...
		killOnce.Do(func() {
			if cmd.Process != nil {
				killErr = ahprocess.KillTree(cmd.Process)
			}
		})
		return killErr
	}
...
// stripClaudePermissionBypassArgs removes user-supplied flags that would
// loosen or bypass the permission posture the wrapper owns (the read-only
// --allowedTools whitelist emitted from filesystem.allowed_paths). A config
// or template author must not be able to silently escalate via raw CLI args;
// widening access should go through an explicit card-schema knob. This is a
// denylist of the known bypass vectors — defense in depth, not a sandbox
// proof.
func stripClaudePermissionBypassArgs(args []string) []string {
	filtered := args[:0]
	skipNext := false
	for _, a := range args {
		if skipNext {
		...
		case a == "--dangerously-skip-permissions":
			continue
		case strings.HasPrefix(a, "--dangerously-skip-permissions="):
			continue
		case a == "--permission-mode":
			skipNext = true // drop flag + its value token
			continue
...
func (s *ClaudeSession) Stream(ctx context.Context, userText string) (<-chan Event, error) {
	...
	case stateInFlight:
		s.mu.Unlock()
		return nil, fmt.Errorf("claude session: %w", ErrTurnInFlight)
	...
	// Enforce cfg.Timeout as a per-turn deadline, mirroring CodexSession's
	// context.WithTimeout semantics. The timer is keyed to this turn's
	// channel — if the turn completes first, the fired callback sees a
	// different (or nil) eventsCh and no-ops, so no bookkeeping is needed.
	if s.cfg.Timeout > 0 {
		time.AfterFunc(s.cfg.Timeout, func() { s.timeoutTurn(ch) })
	}

	// Honour ctx cancellation as a turn abort. Claude's stream-json protocol
	// has no per-turn cancel message, so an external cancel (A2A tasks/cancel,
	// which cancels the request ctx threaded down to here) is realized the same
	// way as a timeout: kill the process. The same stale guard makes this a
	// no-op once the turn has completed — and ctx is always cancelled at stream
	// end, so the watcher goroutine never leaks.
	if ctx != nil {
		go func() {
			<-ctx.Done()
			s.cancelTurn(ch)
...
// abortTurn stops an in-flight turn by killing the process — the only way to
// halt Claude's stream-json protocol, which has no per-turn cancel message. The
// session transitions to EVICTED with sessionID preserved, so the pool resumes
// the conversation via `--resume` on the next access (the same recovery path as
// an unexpected process death). The eventsCh stale guard makes it a no-op once
// the turn has completed, so timeout / cancel / normal-completion races settle
// to exactly one terminal signal.
func (s *ClaudeSession) abortTurn(ch chan Event, cause error) {
	s.mu.Lock()
	if s.eventsCh != ch {
		s.mu.Unlock()
		return
	...
	s.mu.Unlock()
	log.Printf("claude session: turn aborted (%v), killing process (sessionID preserved for resume)", cause)
	ch <- EventTurnDone{Err: cause}
	close(ch)
	// Kill AFTER signalling the caller: the reader goroutine sees EOF next,
	// finds eventsCh already nil, and just confirms the EVICTED state.
	_ = s.transport.kill()
}

// timeoutTurn aborts a turn that exceeded cfg.Timeout.
func (s *ClaudeSession) timeoutTurn(ch chan Event) {
	s.abortTurn(ch, fmt.Errorf("claude turn timed out after %v", s.cfg.Timeout))
}

// cancelTurn aborts a turn whose ctx was cancelled (A2A tasks/cancel, or the
// caller/client disconnecting). ErrTurnCanceled lets the executor map this to a
// CANCELED task state rather than a failure.
func (s *ClaudeSession) cancelTurn(ch chan Event) {
	s.abortTurn(ch, ErrTurnCanceled)
}
...
// emitUser parses a `type: user` envelope — claude echoes tool execution results
// back as a user-role message with tool_result content blocks. Each becomes an
// EventToolResult so callers can observe the outcome of a tool the runtime ran
// internally.
func (s *ClaudeSession) emitUser(ch chan<- Event, raw []byte) {
	var msg struct {
		Message struct {
	...
	for _, part := range msg.Message.Content {
		...
		if err := json.Unmarshal(part, &head); err != nil || head.Type != "tool_result" {
			continue
		}
		var tr struct {
		...
		if err := json.Unmarshal(part, &tr); err == nil {
			ch <- EventToolResult{ToolUseID: tr.ToolUseID, Content: toolResultText(tr.Content), IsError: tr.IsError}
		}
	}
}

// toolResultText flattens a tool_result's content — either a plain string or an
// array of content blocks — to text. Non-text blocks (images/documents) are
// dropped; text covers the overwhelming majority of tool results.
	...
	for _, part := range msg.Message.Content {
		...
		case "tool_use":
			...
			if err := json.Unmarshal(part, &p); err == nil {
				...
				ch <- EventToolUse{Id: p.ID, Name: p.Name, Input: p.Input}
			}
		}
	}
}
