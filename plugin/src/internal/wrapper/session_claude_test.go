...
// TestClaudeSession_TurnTimeoutEvictsSession verifies that cfg.Timeout acts
// as a per-turn deadline (mirroring CodexSession's context.WithTimeout
// semantics): a turn with no result within the deadline gets a synthetic
// EventTurnDone carrying a timeout error, and the session transitions to
// EVICTED (unhealthy) so the pool recreates it via --resume.
func TestClaudeSession_TurnTimeoutEvictsSession(t *testing.T) {
	f := newFakeClaudeTransport()
	defer f.close()

	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
	s, err := newClaudeSessionWithTransport(ctx, SessionConfig{Timeout: 50 * time.Millisecond}, "", f.tr)
	if err != nil {
		t.Fatal(err)
	}
	defer s.Close()

	ch, err := s.Stream(ctx, "hang forever")
	if err != nil {
		t.Fatal(err)
	}
	// Never write a result — the per-turn timer must fire.
	var doneErr error
	for ev := range ch {
		if e, ok := ev.(EventTurnDone); ok {
			doneErr = e.Err
		}
	}
	if doneErr == nil || !strings.Contains(doneErr.Error(), "timed out") {
		t.Fatalf("want timeout error on EventTurnDone, got %v", doneErr)
	}
	if s.IsHealthy() {
		t.Error("session must be unhealthy (evicted) after a turn timeout")
	}
}

// TestClaudeSession_ZeroTimeoutNeverFires pins the cfg.Timeout==0 contract:
// no per-turn deadline is installed and a slow-but-finite turn completes.
func TestClaudeSession_ZeroTimeoutNeverFires(t *testing.T) {
	f := newFakeClaudeTransport()
	defer f.close()
	...
	ch, err := s.Stream(ctx, "slow")
	if err != nil {
		t.Fatal(err)
	}
	go func() {
		time.Sleep(100 * time.Millisecond)
		f.writeLine(t, initEvent("sess-slow"))
		f.writeLine(t, resultOK("sess-slow"))
	}()
	var doneErr error
	for ev := range ch {
		if e, ok := ev.(EventTurnDone); ok {
	...
	if doneErr != nil {
		t.Fatalf("want clean completion with Timeout=0, got %v", doneErr)
	}
	if !s.IsHealthy() {
		t.Error("session must stay healthy after a clean turn")
	}
}

func TestStripClaudePermissionBypassArgs(t *testing.T) {
	tests := []struct {
		name string
		in   []string
		want []string
		...
		{
			name: "strips dangerously-skip-permissions",
			in:   []string{"--dangerously-skip-permissions", "--add-dir=/tmp"},
			want: []string{"--add-dir=/tmp"},
		},
		{
			name: "strips permission-mode with separate value",
			in:   []string{"--permission-mode", "bypassPermissions", "--add-dir=/tmp"},
			want: []string{"--add-dir=/tmp"},
		},
		{
			name: "strips permission-mode eq form",
			in:   []string{"--permission-mode=acceptEdits"},
			want: []string{},
		},
		{
			name: "benign args survive",
			in:   []string{"--add-dir=/tmp", "--allowedTools=Read,LS"},
			want: []string{"--add-dir=/tmp", "--allowedTools=Read,LS"},
		},
	}
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			got := stripClaudePermissionBypassArgs(append([]string(nil), tc.in...))
			if !equalStrings(got, tc.want) {
				t.Errorf("got %v, want %v", got, tc.want)
			}
		})
...
// TestClaudeSession_ConcurrentStreamReturnsBusy pins the same-context
// concurrency contract (specs/2026-06-07-same-context-busy-semantics.md):
// a second Stream while a turn is in flight fails fast with the typed
// ErrTurnInFlight and the user-readable "agent busy:" marker; the first
// turn is unaffected.
func TestClaudeSession_ConcurrentStreamReturnsBusy(t *testing.T) {
	f := newFakeClaudeTransport()
	defer f.close()
	...
	// First turn completes normally despite the rejected intruder.
	go func() {
		f.writeLine(t, initEvent("sess-busy"))
		f.writeLine(t, resultOK("sess-busy"))
	}()
	var doneErr error
	for ev := range ch {
		if e, ok := ev.(EventTurnDone); ok {
	...
	if doneErr != nil {
		t.Errorf("first turn must be unaffected by the busy rejection, got %v", doneErr)
	}
}
