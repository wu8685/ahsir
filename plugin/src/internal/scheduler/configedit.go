...
// config behind.
func AddAgentToConfig(path, name, workspace, workdir string, port int) error {
	...
	newEntry := buildAgentMapping(name, workspace, workdir, port)
...
//
// The temp file is created in the TARGET's directory, not the system tmp
// dir: os.Rename is only atomic (and only guaranteed to work at all) within
// one filesystem, and /tmp is frequently a different mount than the config's
// home.
func writeYAMLNodeAtomic(path string, root *yaml.Node) error {
	tmp, err := os.CreateTemp(filepath.Dir(path), ".ahsir-yaml-*.tmp")
	if err != nil {
		return err
	}
	tmpPath := tmp.Name()
...
func buildAgentMapping(name, workspace, workdir string, port int) *yaml.Node {
	pairs := []*yaml.Node{
	...
	if workdir != "" {
		pairs = append(pairs,
			&yaml.Node{Kind: yaml.ScalarNode, Value: "workdir"},
			&yaml.Node{Kind: yaml.ScalarNode, Value: workdir, Style: yaml.DoubleQuotedStyle},
		)
	}
	if port > 0 {
		pairs = append(pairs,
