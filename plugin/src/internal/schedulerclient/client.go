...
func (c *SchedulerHTTPClient) ChatWithAgent(agentName, contextID, message string) (string, error) {
	return c.ChatWithAgentAs(agentName, contextID, "", message)
}

// ChatWithAgentAs is ChatWithAgent plus speaker attribution: a non-empty
// speaker is posted alongside the message so the scheduler can forward it as
// A2A metadata and record it in the invocation ledger
...
func (c *SchedulerHTTPClient) StreamWithAgent(agentName, contextID, message string, onDelta func(string)) (string, error) {
	return c.StreamWithAgentAs(agentName, contextID, "", message, onDelta)
}

// StreamWithAgentAs is StreamWithAgent plus speaker attribution: a non-empty
...
func (c *SchedulerHTTPClient) StreamWithAgentAs(agentName, contextID, speaker, message string, onDelta func(string)) (string, error) {
	...
	return streamAgentSSE(c.httpc, agentURL, contextID, speaker, message, onDelta)
}
...
// ChatAsync submits a turn without waiting (gateway async=true): returns the
...
func (c *SchedulerHTTPClient) ChatAsync(agentName, contextID, speaker, message string) (taskID, taskContextID string, err error) {
	...
	return out.TaskID, out.ContextID, nil
}

// WaitForTask polls the task until a terminal state and returns the agent's
// reply text. No overall deadline — queue waits can legitimately run long;
// the caller (CLI) owns cancellation via SIGINT.
//
// A 404 mid-poll means the agent restarted (its task store is memory-only):
// only the query handle is lost, not the conversation. The returned error
// names the sanctioned fallback, `ahsir history <agent> <contextId>` — a
// recorded turn there means it executed; an absent one is safe to resend.
func (c *SchedulerHTTPClient) WaitForTask(agentName, taskID, contextID string) (string, error) {
	backoff := 500 * time.Millisecond
	const maxBackoff = 5 * time.Second
	for {
		task, err := c.GetTaskStatus(agentName, taskID)
		if err != nil {
			if strings.Contains(err.Error(), "404") || strings.Contains(err.Error(), "not found") {
				return "", fmt.Errorf("task %s is gone (agent restarted?) — the conversation survives; check `ahsir history %s %s` to see whether the turn ran: %w", taskID, agentName, contextID, err)
			}
			return "", err
		}
		switch task.Status.State {
		case a2a.TaskStateCompleted:
			return lastAgentText(task), nil
...
// lastAgentText extracts the final agent-authored text from a task's history.
func lastAgentText(task *a2a.Task) string {
	for i := len(task.History) - 1; i >= 0; i-- {
		if task.History[i].Role != a2a.MessageRoleAgent {
			continue
		}
		if txt := wrapper.MessageText(task.History[i]); txt != "" {
			return txt
		}
	}
	return ""
...
// GetHistory fetches the per-context transcript through the scheduler proxy.
// Backs `ahsir history <agent> <contextId>` — the replay path for whoever
// takes over a shared context (and the documented fallback when a taskId
// 404s after an agent restart).
func (c *SchedulerHTTPClient) GetHistory(agentName, contextID string) ([]wrapper.TranscriptTurn, error) {
	endpoint := c.baseURL + "/agents/" + url.PathEscape(agentName) + "/history/" + url.PathEscape(contextID)
	resp, err := c.httpc.Get(endpoint)
	if err != nil {
