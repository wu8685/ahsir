...
Run this first to confirm your build, registry, agent card, runtime config, and Anthropic API key are all healthy. Once it works, move on to `../session-reuse/` for the session continuity story.
...
- `claude` CLI on PATH
- `ANTHROPIC_AUTH_TOKEN` env var set to your Anthropic key (`export ANTHROPIC_AUTH_TOKEN=<your-anthropic-key>`)
...
Agent teacher listening on port 9802
