...
- **`ANTHROPIC_AUTH_TOKEN`** env var pointing to an Anthropic API key:

  ```bash
  export ANTHROPIC_AUTH_TOKEN=<your-anthropic-key>
  ```

  The agents are wired to Anthropic's official endpoint (`api.anthropic.com`, model `claude-opus-4-8`). Get a key from <https://console.anthropic.com/>.
...
`port_range.start` (default 9802) in declaration order:
...
```bash
curl -s http://127.0.0.1:9802/healthz
curl -s http://127.0.0.1:9802/readyz
curl -s http://127.0.0.1:9802/.well-known/agent-card.json
```
...
- **Failures**: missing process-start log means `cmd.Start()` failed (check `ANTHROPIC_AUTH_TOKEN`, claude binary path, or the resolved `args` block from `agent-card.yaml`).
...
The integration test uses a mock session — it does **not** require `ANTHROPIC_AUTH_TOKEN` or a real `claude` CLI. To exercise the full path (real LLM, real filesystem reads), use the manual `curl` flow above.
